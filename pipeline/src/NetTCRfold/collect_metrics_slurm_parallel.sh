...
#CONDA_SH must be set in the environment (see configs/envExample.cfg); sbatch inherits it
#from the submitting shell, which sources configs/env.cfg before calling sbatch.

: "${CONDA_SH:?CONDA_SH is not set — define it in configs/env.cfg (see configs/envExample.cfg)}"
source "$CONDA_SH"
