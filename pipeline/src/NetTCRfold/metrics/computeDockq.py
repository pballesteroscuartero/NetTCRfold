...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Path to the DockQ repository",
    )
    parser.add_argument(
        "-n1",
        "--native_chain1",
...
def cif_to_pdb(path_f, pdb_id, model_name):
    #cif_file = pdbx.CIFFile.read(path_f /f"{pdb_id}_{model_name}_model_peptideRenamed.cif")
    cif_file = pdbx.CIFFile.read(path_f /f"{pdb_id}_{model_name}_model.cif")
    model = pdbx.get_structure(cif_file)[0]

    pdb_file = pdb.PDBFile()
    pdb.set_structure(pdb_file, model)
    #pdb_file.write(f"{os.path.splitext(path_f /f'{pdb_id}_{model_name}_model_peptideRenamed.cif')[0]}.pdb")
    pdb_file.write(f"{os.path.splitext(path_f /f'{pdb_id}_{model_name}_model.cif')[0]}.pdb")
    #return f"{os.path.splitext(path_f /f'{pdb_id}_{model_name}_model_peptideRenamed.cif')[0]}.pdb"
    return f"{os.path.splitext(path_f /f'{pdb_id}_{model_name}_model.cif')[0]}.pdb"
...
def compute_dockq(predicted_path: Path, 
    ...
    cmd = f"python3 {dockq_script} {str(predicted_path)} {str(native_path)} -native_chain1 {native_chain1} -model_chain1 {model_chain1} -native_chain2 {native_chain2} -model_chain2 {model_chain2} -no_needle"
...
def dockq_one_complex(predicted_path, native_path, native_chain1, model_chain1, native_chain2, model_chain2, pdb_id, model_name, suffix, dockq_script):
    pdb_path = cif_to_pdb(predicted_path, pdb_id, model_name)
    result = compute_dockq(pdb_path, native_path, native_chain1, model_chain1, native_chain2, model_chain2, dockq_script)
...
def process_complex(args):
    ...
    pdb_id = c.name
    dockqPath=f"{args_ns.dockq_repo}/DockQ.py"
...
def main():
    ...
    with ProcessPoolExecutor(max_workers=os.cpu_count()) as executor:
        list(executor.map(process_complex, tasks))
